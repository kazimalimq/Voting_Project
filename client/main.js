class Poll{
    ...
    async _refresh(){
        ...
        for (const option of data) {
            ...
            template.innerHTML = `
            <div class="poll__option ${this.selected == option.lable? 'poll__option--selected': ''}">
                <div class="poll__option-fill"> </div>
                <div class="poll__option-info">
                    <span class="poll__lable">${option.lable}</span>
        ...
        if (!this.selected){
            fragment.querySelector('.poll__option').addEventListener('click', ()=> {
                fetch(this.endpoint, {
                    method: 'post',
                    body: `add=${option.lable}`,
                    headers :{
                        'Content-Type': 'application/x-www-form-urlencoded'
                    }
                }).then(()=>{
                    this.selected = option.lable;

                    sessionStorage.setItem('poll-selected', option.lable)
                    this._refresh();
                })
            })
        }
...
const p = new Poll(
    document.querySelector('.poll'), 'Which do you prefer?'
);
